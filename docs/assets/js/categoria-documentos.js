...
function openForm(doc = null) {
  ...
  formTitle.textContent = doc ? `Editar ${CATEGORY.tipo.toLowerCase()}` : `Nuevo ${CATEGORY.tipo.toLowerCase()}`;
  modal.hidden = false;
}

async function meta() {
  const d = await apiGet('/api/documentos/meta');
  procesos = d.procesos || [];
  areas = d.areas || [];
  fillMeta();
}
...
function links(doc) {
  const entries = [
    ['Abrir documento', doc.enlace_doc, 'doc'],
    ['Flujograma 1', doc.enlace_fluj, 'flow'],
    ['Flujograma 2', doc.enlace_fluj1, 'flow'],
    ['Flujograma 3', doc.enlace_fluj2, 'flow'],
    ['Flujograma 4', doc.enlace_fluj3, 'flow'],
  ].map(([label, url, type]) => [label, safeUrl(url), type]).filter(([, url]) => url);

  if (!entries.length) return '<span class="muted">Sin archivos cargados</span>';
...
async function load() {
  ...
  try {
    ...
    const d = await apiGet(`/api/documentos?${p}`);
    cache = d.documentos || [];
    rows.innerHTML = cache.length ? cache.map(x => `<tr>
      <td><div class="cell-title"><strong>${esc(x.nombre)}</strong>${x.version ? `<span class="cell-sub">Versión ${esc(x.version)}</span>` : ''}</div></td>
      ...
      <td class="files-cell">${links(x)}</td>
...
async function removeD(did) {
  if (!confirm(`¿Eliminar este ${CATEGORY.tipo.toLowerCase()}?`)) return;
  try { await apiDelete(`/api/documentos/${did}`); showMsg(`${CATEGORY.tipo} eliminado`); await load(); }
...
form.addEventListener('submit', async e => {
  e.preventDefault();
  const body = {
    proceso_id: proceso_id.value, tipo: CATEGORY.tipo, nombre: nombre.value, version: version.value,
    estado: estado.value, fecha_actualizacion: fecha_actualizacion.value, enlace_doc: enlace_doc.value,
    enlace_fluj: enlace_fluj.value, enlace_fluj1: enlace_fluj1.value, enlace_fluj2: enlace_fluj2.value,
    enlace_fluj3: enlace_fluj3.value
  };
  try {
    id.value ? await apiPut(`/api/documentos/${id.value}`, body) : await apiPost('/api/documentos', body);
    closeForm(); showMsg(`${CATEGORY.tipo} guardado`); await load();
  } catch (e) { showMsg(e.message, 'error'); }
});
